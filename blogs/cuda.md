...
GEMM is D = alpha * A * B + beta * C.

On the surface level, multiplying two matrices is one of the simplest algoritms but it starts to show its beauty as you dig deep into it and try to parallelize the algoritm.

---

### Inner Product Formulation

The standard formulation of matrix multiplication is given by
...
Alternatively, matrix multiplication can be computed as a sum of outer products. This is expressed as
...
Each outer product $ A_{:,k} B_{k,:} $ produces an $ M \times N $ rank-1 matrix, and then all these are summed to form $ C $.

For each $ k $, you load:
- $ M $ elements for $ A_{:,k} $,
- $ N $ elements for $ B_{k,:} $.

Thus, the number of memory loads per outer product is

$$
M + N.
$$

Since there are $ K $ such outer products, the total number of memory loads is
...
- Inner Product:
  ...
  $$
  \text{Memory Loads}_{\text{inner}} \approx 2 \times 100 \times 100 \times 100 = 2\,000\,000 \text{ loads.}
  $$

- Outer Product:

  $$
  \text{Memory Loads}_{\text{outer}} \approx 100 \times (100 + 100) = 100 \times 200 = 20\,000 \text{ loads.}
  $$

This example illustrates that the outer product formulation can potentially lead to a significant reduction in memory loads. In practice, high-performance implementations (e.g., in GEMM routines) further optimize these operations using blocking and cache reuse techniques to minimize memory traffic.

---

This detailed but to-the-point explanation highlights both formulations with their corresponding memory load calculations using LaTeX.
...
Matmul is divided into these steps:
...
- Intro video: https://www.youtube.com/watch?v=HOVvQfcBMTQ


### Compiling CUDA code

nvcc does code-generation in two stages:
...
2. Back-end	SASS / cubin for one or more real GPUs (“sm XX…”) - or more PTX if you ask for it	code= inside -gencode (or --gpu-code)
arch=compute_90a
...
code=sm_90a
Asks the back-end (ptxas) to turn that PTX into native machine code for SM 90a.
...
nvcc -gencode arch=compute_90a,code=\"sm_90a,compute_90a\"  file.cu
which keeps both:
...
compute_XX[a] = virtual architecture ➜ PTX feature set only.
Must be used on the arch= side.
...
“The arch= clause… must always be a PTX version.
The code= clause specifies the back-end target and can be cubin or PTX or both.” 
NVIDIA Docs
and

“--gpu-architecture takes a virtual compute architecture, while
--gpu-code takes a list of actual GPUs.” 
NVIDIA Docs
Shorthands you might see
-arch=sm_90a expands to
-gencode arch=compute_90a,code=\"sm_90a,compute_90a\" 
NVIDIA Docs
You can repeat -gencode to include several GPU generations in one binary:
nvcc \
  -gencode arch=compute_90a,code=\"sm_90a,compute_90a\" \
...
arch= = “what PTX ISA do I compile for?”
code= = “what do I embed in the executable – ready-to-run cubin, PTX, or both – and for which GPUs?”
Using -gencode arch=compute_90a,code=sm_90a therefore means:

“Compile with Hopper-90a features and pre-assemble the result for GPUs whose SM is 90a.”
...
Explanation:
compute_XX:
This flag specifies the compute capability of the target virtual architecture for PTX code generation. PTX is a low-level parallel thread execution instruction set architecture that acts as an intermediary between the CUDA source code and the specific GPU hardware. 
sm_XX:
This flag specifies the compute capability of the target GPU architecture for machine code (SASS) generation. SASS is the actual machine code that the GPU executes. 
Fat Binaries:
CUDA compilers like nvcc can generate fat binaries that contain both PTX code for future compatibility and SASS code for specific architectures. 
Compilation Process:
...
Best Practices:
To ensure maximum compatibility and performance, it's recommended to include SASS code for all target architectures and PTX code for the latest architecture. 
Example:
If you compile with:
...
This will: 
Generate PTX code for compute capability 3.0, 5.2, and 7.0.
Generate SASS code for compute capability 3.0, 5.2, and 7.0.
Embed the PTX code for compute capability 7.0 (latest architecture) in the binary for future compatibility.
When running on a GPU with compute capability 7.0, the SASS code for 7.0 will be used. If the target GPU has a different compute capability, the PTX code will be JIT compiled to the appropriate SASS


- compute_XX refers to a PTX version and sm_XX refers to a cubin version…
the arch= clause must always be a PTX version, while the code= clause can be cubin or PTX or both
...
### Nice Blog on sync operations
https://medium.com/@fatlip/cuda-sync-co-b475c3dbd57f
