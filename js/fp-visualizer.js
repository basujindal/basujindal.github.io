...
const blockMaxInput = el('blockMaxInput');
const scaleBadgeEl = el('scaleBadge');
...
function renderPropsTable(x, dec) {
  ...
  if (data.some(d => d.def.isMX)) {
    const blockMax = Math.abs(safeParseNumber(blockMaxInput.value));
    html += `<tr class="fp-section-row"><td colspan="${colCount}">MX block scaling</td></tr>`;
    renderRows([
      { label: 'Block max',         fn: d => d.def.isMX ? fmtNumber(blockMax, dec) : '\u2014' },
      { label: 'Shared scale X',    fn: d => d.def.isMX ? `2^${Math.round(Math.log2(d.def.scale))} = ${fmtNumber(d.def.scale, dec)}` : '\u2014' },
      { label: 'Scale (E8M0)',      fn: d => { if (!d.def.isMX) return '\u2014'; const f = Math.max(0, Math.min(254, Math.round(Math.log2(d.def.scale)) + 127)); return `${hex(f, 2)} ${binStr(f, 8)}`; } },
      { label: 'Stored elem (FP8)', fn: d => d.def.isMX ? fmtNumber(d.qv / d.def.scale, dec) : '\u2014' },
      { label: 'Amortized bits',    fn: d => d.def.isMX ? `${d.def.bits + d.def.bits / d.def.blockSize} (${d.def.bits} + ${d.def.bits}/${d.def.blockSize})` : '\u2014' },
    ]);
  }
...
function getThemeColors() {
  ...
  return {
    ...
    mxfp8:    isDark ? 'rgba(60, 210, 235, 0.40)'  : 'rgba(20, 180, 210, 0.35)',
...
// ---------- MX block scale ----------
// Recompute each MX format's shared scale X from the Block max input, rebuild its
// scaled grid, and reflect X (and any x-out-of-block hint) in the badge.
function updateMXScale() {
  const blockMax = Math.abs(safeParseNumber(blockMaxInput.value));
  const x = safeParseNumber(xInput.value);
  let badged = false;
  for (const def of FORMAT_DEFS) {
    if (!def.isMX) continue;
    applyMXScale(def, computeMXScale(blockMax, def.blockEmax));
    if (!badged && scaleBadgeEl) {
      const k = Math.round(Math.log2(def.scale));
      let html = `X = 2<sup>${k}</sup>`;
      if (Number.isFinite(x) && blockMax > 0 && Math.abs(x) > blockMax) {
        html += ` <span class="fp-sat">&middot; |x|&gt;max</span>`;
      }
      scaleBadgeEl.innerHTML = html;
      badged = true;
    }
  }
}
...
blockMaxInput.addEventListener('input', scheduleUpdate);
