...
// ---------- MX block-scaled formats ----------
// An MX value is an FP8 element times a shared power-of-two scale X. The element
// grid never changes, so we stash it once (unscaled) and rebuild the displayed
// TABLES/DTYPE_STATS entries as element*X whenever the block scale changes.
const MX_BASE = {};
FORMAT_DEFS.forEach(def => {
  if (def.isMX) MX_BASE[def.key] = TABLES[def.key].entries.map(e => ({ bits: e.bits, value: e.value }));
});

function applyMXScale(def, scale) {
  if (!def.isMX) return;
  def.scale = scale;
  // scale > 0, so multiplying preserves the existing sort order.
  TABLES[def.key].entries = MX_BASE[def.key].map(e => ({ bits: e.bits, value: e.value * scale }));
  DTYPE_STATS[def.key] = computeDtypeStats(def);
}
